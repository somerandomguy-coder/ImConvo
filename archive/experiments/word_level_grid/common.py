"""Shared constants and helpers for the GRID word-level experiment."""
...
def make_run_id(model_variant: str, frontend_model: str, augmentation_profile: str) -> str:
    """Generate a compact run identifier."""
    timestamp = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    safe_variant = re.sub(r"[^a-zA-Z0-9]+", "-", model_variant.lower()).strip("-")
    safe_frontend = re.sub(r"[^a-zA-Z0-9]+", "-", frontend_model.lower()).strip("-")
    safe_aug = re.sub(r"[^a-zA-Z0-9]+", "-", augmentation_profile.lower()).strip("-")
    return f"{timestamp}_wordgrid_{safe_variant}_{safe_frontend}_{safe_aug}"
...
def make_checkpoint_filename(
    ...
    """Annotated checkpoint filename for word-level experiments."""
    safe_variant = re.sub(r"[^a-zA-Z0-9]+", "-", model_variant.lower()).strip("-")
    safe_frontend = re.sub(r"[^a-zA-Z0-9]+", "-", frontend_model.lower()).strip("-")
    safe_aug = re.sub(r"[^a-zA-Z0-9]+", "-", augmentation_profile.lower()).strip("-")
    return (
        f"{run_id}_mv-{safe_variant}_fe-{safe_frontend}_aug-{safe_aug}_"
        f"bs-{int(batch_size)}_seed-{int(seed)}.weights.h5"
    )
